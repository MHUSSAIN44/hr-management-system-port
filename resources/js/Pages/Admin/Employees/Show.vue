...
<script setup>
    ...
    // Check if any documents are expiring soon (60 days for admin)
    const hasExpiringDocuments = computed(() => {
        ...
        ].some(date => date && new Date(date) < sixtyDaysFromNow);
    });

    // Check for critically expiring documents (30 days)
    const hasCriticallyExpiringDocuments = computed(() => {
        const now = new Date();
    ...
    const getExpiryStatus = (dateString, criticalDays = 30, warningDays = 60) => {
        if (!dateString) return null;

        const expiryDate = new Date(dateString);
        const now = new Date();
        const criticalDate = new Date(Date.now() + criticalDays*24*60*60*1000);
        ...
        if (expiryDate < now) {
            ...
            return { status: 'critical', color: 'bg-red-100 text-red-800', label: 'Expiring Soon' };
        } else if (expiryDate < warningDate) {
            return { status: 'warning', color: 'bg-yellow-100 text-yellow-800', label: 'Renewal Due' };
        }
        return null;
    };
</script>
    ...
    <AdminLayout>
        <template #header>
            <div class="flex justify-between items-center">
                ...
                <div class="flex space-x-3">
                    <Link :href="route('admin.employees.edit', employee.id)" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 active:bg-blue-800 focus:outline-none focus:ring focus:ring-blue-200 transition">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                        </svg>
                        Edit
                    </Link>
                    <Link :href="route('admin.employees.index')" class="inline-flex items-center px-4 py-2 bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-300 active:bg-gray-300 focus:outline-none focus:ring focus:ring-gray-200 transition">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <!-- Critical Document Expiry Alert (30 days) -->
                <div v-if="hasCriticallyExpiringDocuments" class="bg-red-50 border border-red-200 rounded-md p-4 mb-6">
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <h3 class="text-sm font-medium text-red-800">🚨 CRITICAL: Immediate Document Renewal Required</h3>
                            <div class="mt-2 text-sm text-red-700">
                                <p><strong>{{ employee.employee_name }}</strong> has critical documents expiring within 30 days. Take immediate action!</p>
                                <ul class="list-disc pl-5 mt-2 space-y-1">
                                    <li v-if="employee.passport_expiry && new Date(employee.passport_expiry) < new Date(Date.now() + 30*24*60*60*1000)">
                                        <strong>Passport expires on {{ formatDate(employee.passport_expiry) }}</strong> - Contact employee immediately
                                    </li>
                                    <li v-if="employee.visa_expiry_date && new Date(employee.visa_expiry_date) < new Date(Date.now() + 30*24*60*60*1000)">
                                        <strong>Visa expires on {{ formatDate(employee.visa_expiry_date) }}</strong> - Legal compliance required
                                    </li>
                                </ul>
                            </div>
                        </div>
                ...
                <!-- General Document Expiry Alert (60 days) -->
                <div v-if="hasExpiringDocuments && !hasCriticallyExpiringDocuments" class="bg-yellow-50 border border-yellow-200 rounded-md p-4 mb-6">
                    <div class="flex">
                        <div class="flex-shrink-0">
                        ...
                        <div class="ml-3">
                            <h3 class="text-sm font-medium text-yellow-800">Document Renewal Planning Required</h3>
                            <div class="mt-2 text-sm text-yellow-700">
                                <p><strong>{{ employee.employee_name }}</strong> has documents expiring within 60 days. Plan for renewal:</p>
                                <ul class="list-disc pl-5 mt-2 space-y-1">
                                    <li v-if="employee.eid_expiry_date && new Date(employee.eid_expiry_date) < new Date(Date.now() + 60*24*60*60*1000)">
                                        EID expires on {{ formatDate(employee.eid_expiry_date) }}
                                    </li>
                                    <li v-if="employee.insurance_expiry_date && new Date(employee.insurance_expiry_date) < new Date(Date.now() + 60*24*60*60*1000)">
                                        Insurance expires on {{ formatDate(employee.insurance_expiry_date) }}
                                    </li>
                                    <li v-if="employee.malpractice_expiry_date && new Date(employee.malpractice_expiry_date) < new Date(Date.now() + 60*24*60*60*1000)">
                                        Malpractice insurance expires on {{ formatDate(employee.malpractice_expiry_date) }}
                                    </li>
                                    <li v-if="employee.driving_license_expiry && new Date(employee.driving_license_expiry) < new Date(Date.now() + 60*24*60*60*1000)">
                                        Driving license expires on {{ formatDate(employee.driving_license_expiry) }}
                                    </li>
                                    <li v-if="employee.passport_expiry && new Date(employee.passport_expiry) < new Date(Date.now() + 60*24*60*60*1000) && new Date(employee.passport_expiry) >= new Date(Date.now() + 30*24*60*60*1000)">
                                        Passport expires on {{ formatDate(employee.passport_expiry) }}
                                    </li>
                                    <li v-if="employee.visa_expiry_date && new Date(employee.visa_expiry_date) < new Date(Date.now() + 60*24*60*60*1000) && new Date(employee.visa_expiry_date) >= new Date(Date.now() + 30*24*60*60*1000)">
                                ...
                                <div>
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900">
                                        <span v-if="employee.passport_expiry">
                                            ...
                                            <span v-if="getExpiryStatus(employee.passport_expiry)" class="ml-2 inline-flex items-center px-2 py-1 text-xs font-medium rounded-full" :class="getExpiryStatus(employee.passport_expiry).color">
                                                {{ getExpiryStatus(employee.passport_expiry).label }}
                                            </span>
                                        </span>
                                ...
                                <div>
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900">
                                        <span v-if="employee.visa_expiry_date">
                                            ...
                                            <span v-if="getExpiryStatus(employee.visa_expiry_date)" class="ml-2 inline-flex items-center px-2 py-1 text-xs font-medium rounded-full" :class="getExpiryStatus(employee.visa_expiry_date).color">
                                                {{ getExpiryStatus(employee.visa_expiry_date).label }}
                                            </span>
                                        </span>
                                        <span v-else class="text-gray-400 italic">Not provided</span>
                                    </dd>
                                ...
                                <div>
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900">
                                        <span v-if="employee.driving_license_expiry">
                                            ...
                                            <span v-if="getExpiryStatus(employee.driving_license_expiry)" class="ml-2 inline-flex items-center px-2 py-1 text-xs font-medium rounded-full" :class="getExpiryStatus(employee.driving_license_expiry).color">
                                                {{ getExpiryStatus(employee.driving_license_expiry).label }}
                                            </span>
                                        </span>
                                        <span v-else class="text-gray-400 italic">Not provided</span>
                                    </dd>
                                ...
                                <div>
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900">
                                        <span v-if="employee.eid_expiry_date">
                                            ...
                                            <span v-if="getExpiryStatus(employee.eid_expiry_date)" class="ml-2 inline-flex items-center px-2 py-1 text-xs font-medium rounded-full" :class="getExpiryStatus(employee.eid_expiry_date).color">
                                                {{ getExpiryStatus(employee.eid_expiry_date).label }}
                                            </span>
                                        </span>
                                        <span v-else class="text-gray-400 italic">Not provided</span>
                                    </dd>
                                ...
                                <div>
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900">
                                        <span v-if="employee.insurance_expiry_date">
                                            ...
                                            <span v-if="getExpiryStatus(employee.insurance_expiry_date)" class="ml-2 inline-flex items-center px-2 py-1 text-xs font-medium rounded-full" :class="getExpiryStatus(employee.insurance_expiry_date).color">
                                                {{ getExpiryStatus(employee.insurance_expiry_date).label }}
                                            </span>
                                        </span>
                                        <span v-else class="text-gray-400 italic">Not provided</span>
                                    </dd>
                                ...
                                <div>
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900">
                                        <span v-if="employee.malpractice_expiry_date">
                                            ...
                                            <span v-if="getExpiryStatus(employee.malpractice_expiry_date)" class="ml-2 inline-flex items-center px-2 py-1 text-xs font-medium rounded-full" :class="getExpiryStatus(employee.malpractice_expiry_date).color">
                                                {{ getExpiryStatus(employee.malpractice_expiry_date).label }}
                                            </span>
                                        </span>
                                        <span v-else class="text-gray-400 italic">Not provided</span>
                                    </dd>
                            ...
                            <dl class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                                <div>
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900">
                                        ...
                                        <span v-else-if="employee.annual_leave_balance > 20" class="ml-2 inline-flex items-center px-2 py-1 text-xs font-medium bg-blue-100 text-blue-800 rounded-full">
                                            High Balance
                                        </span>
                                    </dd>
                                </div>
                                <div>
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900">
                                        ...
                                        <span v-if="employee.medical_leave_balance < 3" class="ml-2 inline-flex items-center px-2 py-1 text-xs font-medium bg-red-100 text-red-800 rounded-full">
                                            Critical
                                        </span>
                                        <span v-else-if="employee.medical_leave_balance < 7" class="ml-2 inline-flex items-center px-2 py-1 text-xs font-medium bg-yellow-100 text-yellow-800 rounded-full">
                                            Low Balance
                                        </span>
                                    </dd>
                                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6" v-if="employee.documents?.length > 0 || employee.assets?.length > 0">
                    ...
                    <div v-if="employee.documents?.length > 0" class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        ...
                        <div class="p-6">
                            <div class="space-y-3">
                                <div v-for="document in employee.documents" :key="document.id" class="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200">
                                    <div class="flex-1">
                                        <div class="text-sm font-medium text-gray-900">{{ document.title }}</div>
                                        <div class="text-sm text-gray-500">
                                            <span v-if="document.expiry_date">
                                                Expires: {{ formatDate(document.expiry_date) }}
                                                <span v-if="getExpiryStatus(document.expiry_date)" class="ml-2 inline-flex items-center px-2 py-1 text-xs font-medium rounded-full" :class="getExpiryStatus(document.expiry_date).color">
                                    ...
                                    <a :href="`/storage/${document.file_path}`" target="_blank" class="text-blue-600 hover:text-blue-900 ml-4">
                    ...
                    <div v-if="employee.assets?.length > 0" class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        ...
                        <div class="p-6">
                            <div class="space-y-3">
                                <div v-for="asset in employee.assets" :key="asset.id" class="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200">
                ...
                <div class="mt-6">
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
                            <h3 class="text-lg font-medium text-gray-900">Admin Action Required</h3>
                        </div>
                        <div class="p-6">
                            <div class="space-y-4">
                                ...
                                <div v-if="employee.passport_expiry && new Date(employee.passport_expiry) < new Date(Date.now() + 30*24*60*60*1000)" class="flex items-center p-3 bg-red-50 border border-red-200 rounded-lg">
                                    ...
                                    <div class="ml-3 flex-1">
                                        <p class="text-sm font-medium text-red-800">🚨 URGENT: Contact employee for passport renewal</p>
                                        <p class="text-xs text-red-600">Passport expires {{ formatDate(employee.passport_expiry) }}. Immediate action required.</p>
                                    </div>
                                </div>
                                ...
                                <div v-if="employee.visa_expiry_date && new Date(employee.visa_expiry_date) < new Date(Date.now() + 30*24*60*60*1000)" class="flex items-center p-3 bg-red-50 border border-red-200 rounded-lg">
                                    ...
                                    <div class="ml-3 flex-1">
                                        <p class="text-sm font-medium text-red-800">🚨 CRITICAL: Visa renewal process must start</p>
                                        <p class="text-xs text-red-600">Visa expires {{ formatDate(employee.visa_expiry_date) }}. Legal compliance at risk.</p>
                                    </div>
                                </div>

                                <!-- Warning Actions (60 days) -->
                                <div v-if="employee.eid_expiry_date && new Date(employee.eid_expiry_date) < new Date(Date.now() + 60*24*60*60*1000) && new Date(employee.eid_expiry_date) >= new Date(Date.now() + 30*24*60*60*1000)" class="flex items-center p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                                    <div class="flex-shrink-0">
                                    ...
                                    <div class="ml-3 flex-1">
                                        <p class="text-sm font-medium text-yellow-800">Schedule EID renewal reminder</p>
                                        <p class="text-xs text-yellow-600">EID expires {{ formatDate(employee.eid_expiry_date) }}. Send renewal reminder to employee.</p>
                                    </div>
                                </div>
                                ...
                                <div v-if="employee.driving_license_expiry && new Date(employee.driving_license_expiry) < new Date(Date.now() + 60*24*60*60*1000)" class="flex items-center p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                                    ...
                                    <div class="ml-3 flex-1">
                                        <p class="text-sm font-medium text-yellow-800">Driver license renewal notification</p>
                                        <p class="text-xs text-yellow-600">License expires {{ formatDate(employee.driving_license_expiry) }}. Notify employee for renewal.</p>
                                    </div>
                                </div>

                                <div v-if="employee.insurance_expiry_date && new Date(employee.insurance_expiry_date) < new Date(Date.now() + 60*24*60*60*1000)" class="flex items-center p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                                    <div class="flex-shrink-0">
                                        <svg class="h-5 w-5 text-yellow-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z" />
                                        </svg>
                                    ...
                                    <div class="ml-3 flex-1">
                                        <p class="text-sm font-medium text-yellow-800">Insurance renewal required</p>
                                        <p class="text-xs text-yellow-600">Insurance expires {{ formatDate(employee.insurance_expiry_date) }}. Coordinate renewal process.</p>
                                    </div>
                                </div>

                                <!-- Leave Balance Actions -->
                                <div v-if="employee.annual_leave_balance > 25" class="flex items-center p-3 bg-blue-50 border border-blue-200 rounded-lg">
                                    <div class="flex-shrink-0">
                                    ...
                                    <div class="ml-3 flex-1">
                                        <p class="text-sm font-medium text-blue-800">High leave balance requires attention</p>
                                        <p class="text-xs text-blue-600">Employee has {{ employee.annual_leave_balance }} days.</p>
                                    </div>
                                </div>

                                <div v-if="employee.medical_leave_balance < 3" class="flex items-center p-3 bg-orange-50 border border-orange-200 rounded-lg">
                                    <div class="flex-shrink-0">
                                    ...
                                    <div class="ml-3 flex-1">
                                        <p class="text-sm font-medium text-orange-800">Low medical leave balance</p>
                                        <p class="text-xs text-orange-600">Only {{ employee.medical_leave_balance }} days remaining. Monitor usage carefully.</p>
                                    </div>
                                </div>

                                <!-- No actions required message -->
                                <div v-if="!hasExpiringDocuments && employee.annual_leave_balance <= 25 && employee.medical_leave_balance >= 3" class="text-center py-8">
                                    <svg class="mx-auto h-12 w-12 text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                    <h3 class="mt-2 text-sm font-medium text-gray-900">No Immediate Actions Required</h3>
                                    <p class="mt-1 text-sm text-gray-500">{{ employee.employee_name }}'s profile is up to date. All documents and balances are in good standing.</p>
                                </div>
                            </div>
                        </div>
                    </div>
