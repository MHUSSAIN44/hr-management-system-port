<!--// resources/js/Pages/Manager/Attendance/Index.vue-->
<script setup>
    ...
    const checkIn = async () => {
        ...
        try {
            ...
            const response = await fetch(route('manager.attendance.checkin'), {
                method: 'POST',
                headers: {
    ...
    const checkOut = async () => {
        ...
        try {
            ...
            const response = await fetch(route('manager.attendance.checkout'), {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
    ...
    const applyFilters = () => {
        router.get(route('manager.attendance.index'), {
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tr v-for="record in attendance.data" :key="record.id" class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <Link :href="route('manager.attendance.show', record.id)" class="text-blue-600 hover:text-blue-900" title="View Details">
