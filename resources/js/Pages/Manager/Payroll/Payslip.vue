<!-- resources/js/Pages/Manager/Payroll/Payslip.vue -->
<script setup>
...
<template>
    <Head :title="`Manager Payslip - ${getMonthName(payroll?.month)} ${payroll?.year}`" />

    <ManagerLayout>
        <template #header>
            <div class="flex justify-between items-center print:hidden">
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">My Manager Payslip</h2>
                <div class="flex space-x-3">
                    ...
                    <Link :href="route('manager.payroll.show', payroll.id)" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 transition">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="bg-white shadow-sm print:shadow-none print:border-0" style="min-height: 11in;">
                    <div class="p-8 print:p-6">
                        ...
                        <div class="text-center border-b-2 border-gray-300 pb-6 mb-6">
                            ...
                            <p class="text-gray-600">Office 111 CEO building executive Offices, DIP-1, Dubai</p>
                        ...
                        <div class="flex justify-between items-center mb-8">
                            <div>
                                <h2 class="text-2xl font-bold text-gray-900 mb-2">MANAGER SALARY SLIP</h2>
                        ...
                        <!-- Manager Information -->
                        <div class="grid grid-cols-2 gap-8 mb-8 p-4 bg-green-50 rounded-lg border border-green-200">
                            <div>
                                <h3 class="text-lg font-semibold text-green-900 mb-3">Manager Details</h3>
                                <div class="space-y-2">
                                    ...
                                    <div class="flex justify-between">
                                        <span class="text-sm font-medium text-gray-600">Manager ID:</span>
                                        <span class="text-sm text-gray-900">MGR-{{ String(payroll?.employee?.id).padStart(4, '0') }}</span>
                                    </div>
                                    <div class="flex justify-between">
                            ...
                            <div>
                                <h3 class="text-lg font-semibold text-green-900 mb-3">Payment Details</h3>
                                <div class="space-y-2">
                                    <div class="flex justify-between">
                        ...
                        <div class="grid grid-cols-2 gap-8 mb-8">
                            <!-- Earnings -->
                            <div>
                                <h3 class="text-lg font-semibold text-gray-900 mb-4 border-b border-gray-300 pb-2">MANAGER EARNINGS</h3>
                                <div class="space-y-3">
                                    <div class="flex justify-between py-1">
                                        <span class="text-sm text-gray-700">Basic Salary</span>
                                        <span class="text-sm font-medium text-gray-900">{{ formatCurrency(payroll?.basic_salary || 0) }}</span>
                                    </div>
                                    <div class="flex justify-between py-1">
                                        <span class="text-sm text-gray-700">Accommodation Allowance</span>
                                        <span class="text-sm font-medium text-gray-900">{{ formatCurrency(payroll?.accommodation || 0) }}</span>
                                    </div>
                                    <div class="flex justify-between py-1">
                                        <span class="text-sm text-gray-700">Management Allowances</span>
                                        <span class="text-sm font-medium text-gray-900">{{ formatCurrency(payroll?.allowances || 0) }}</span>
                                    </div>
                                    <div class="flex justify-between py-1">
                                        <span class="text-sm text-gray-700">Overtime Amount</span>
                                        <span class="text-sm font-medium text-gray-900">{{ formatCurrency(payroll?.overtime_amount || 0) }}</span>
                                    </div>
                                    <div class="border-t border-gray-300 pt-2 mt-3">
                                        <div class="flex justify-between">
                                            <span class="text-sm font-semibold text-gray-900">TOTAL EARNINGS</span>
                                            <span class="text-sm font-bold text-green-600">{{ formatCurrency(payroll?.gross_salary || 0) }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="space-y-3">
                                    <div class="flex justify-between py-1">
                                        ...
                                        <span class="text-sm font-medium text-gray-900">{{ formatCurrency(payroll?.deductions || 0) }}</span>
                                    </div>
                                    <div class="border-t border-gray-300 pt-2 mt-3">
                                        <div class="flex justify-between">
                                            <span class="text-sm font-semibold text-gray-900">TOTAL DEDUCTIONS</span>
                                            <span class="text-sm font-bold text-red-600">{{ formatCurrency(payroll?.deductions || 0) }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="bg-green-50 p-6 rounded-lg mb-8 border border-green-200">
                            <div class="grid grid-cols-3 gap-4">
                                <div class="text-center">
                                    ...
                                    <p class="text-xl font-bold text-green-600">{{ formatCurrency(payroll?.gross_salary || 0) }}</p>
                                </div>
                                <div class="text-center">
                                    <p class="text-sm text-gray-600">Total Deductions</p>
                                    <p class="text-xl font-bold text-red-600">{{ formatCurrency(payroll?.deductions || 0) }}</p>
                                </div>
                                <div class="text-center border-l-2 border-green-300">
                                    <p class="text-sm text-gray-600">MANAGER NET PAY</p>
                                    <p class="text-2xl font-bold text-green-600">{{ formatCurrency(payroll?.net_salary || 0) }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="border-t-2 border-gray-300 pt-6">
                            <div class="grid grid-cols-2 gap-8">
                                <div>
                                    <p class="text-sm text-gray-600 mb-4">This is a computer generated manager payslip and does not require signature.</p>
                                    <p class="text-xs text-gray-500">
                                        <strong>Note:</strong> Please keep this payslip for your records.
                                        For any queries regarding your manager salary, please contact the HR department.
                                    </p>

                                    <p class="text-xs text-gray-500 mt-1">
                                        <strong>Confidential:</strong> This document contains sensitive financial information and should be kept secure.
                                    </p>
                                </div>
                                <div class="text-right">
                                    <div class="mt-8">
                        ...
                        <div class="text-center mt-8 text-xs text-gray-400 print:block hidden">
                            <p>Generated on {{ formatDate(new Date()) }} | Manager Payslip - Company Confidential</p>
                        </div>
                    </div>
                </div>
            </div>
