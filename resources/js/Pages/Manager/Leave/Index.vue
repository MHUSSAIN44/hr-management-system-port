<!--// resources/js/Pages/Manager/Leave/Index.vue-->
<script setup>
    ...
    watch([search, perPage, statusFilter, leaveTypeFilter], () => {
        router.get(route('manager.leave.index'), {
    ...
    const approveLeave = () => {
        approveForm.post(route('manager.leave.approve', currentLeave.value.id), {
            onSuccess: () => {
                closeModals();
            }
        });
    ...
    const rejectLeave = () => {
        rejectForm.post(route('manager.leave.reject', currentLeave.value.id), {
            onSuccess: () => {
                closeModals();
            }
        });
    ...
    <ManagerLayout>
        <template #header>
            <div class="flex justify-between items-center">
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Team Leave Requests</h2>
                <div class="flex space-x-3">
                    <Link :href="route('manager.leave.my')" class="inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700 transition">
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                    <div class="p-6 bg-white border-b border-gray-200">
                        <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                            ...
                            <div>
                                <FilterDropdown v-model="leaveTypeFilter" label="Leave Type" :options="[
                    ...
                    <div class="p-6">
                        <div class="space-y-6">
                                ...
                                <div class="flex items-start justify-between">
                                    <div class="flex-1">
                                        ...
                                        <div class="flex items-center space-x-3 mb-3">
                                            ...
                                            <div>
                                                <h3 class="text-lg font-medium text-gray-900">{{ leave.employee.employee_name }}</h3>
                                                <p class="text-sm text-gray-500">{{ leave.employee.designation?.name }} - {{ leave.employee.department?.name }}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                                            ...
                                            <div>
                                                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium" :class="getStatusColor(leave.manager_status)">
                                        ...
                                        <div v-if="leave.manager_comments" class="bg-blue-50 border-l-4 border-blue-400 p-3">
                                            <p class="text-sm font-medium text-blue-800">Your Comments</p>
                                            <p class="text-sm text-blue-700">{{ leave.manager_comments }}</p>
                                        </div>
                                    </div>

                                    <!-- Actions -->
                                    <div v-if="leave.manager_status === 'pending'" class="ml-4 flex flex-col space-y-2">
                                        <button @click="openApproveModal(leave)" class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                                            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                                            </svg>
                                            Approve
                                        </button>
                                        <button @click="openRejectModal(leave)" class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                                            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                                            </svg>
                                            Reject
                            ...
                            <div v-if="leaves.data.length === 0" class="text-center py-12">
                                ...
                                <p class="mt-1 text-sm text-gray-500">No team members have submitted leave requests yet.</p>
                            </div>
                        </div>
